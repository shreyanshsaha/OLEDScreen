...
void OLEDText::printText(char* text, int16_t x, int16_t y){
	printText(text, x, y, this->config);
}

void OLEDText::printText(char* text, int16_t x, int16_t y, TextConfig config){
...
void OLEDText::printText(char* text){
	display->setTextSize(config.textSize);
	display->setTextColor(config.textColor);
	display->setTextColor(config.textColor, config.bgColor);
	display->println(text);
